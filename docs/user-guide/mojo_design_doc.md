...
MOJO provides a comprehensive navigation system with two complementary approaches optimized for modern web applications:

### Primary: href-based Navigation (SEO-Friendly)

Standard HTML navigation with proper `href` attributes:
...
**Benefits:**
- ✅ Copy link support (right-click → copy link)
- ✅ SEO crawlable navigation structure
- ✅ Browser features work (Ctrl+click, middle-click)
- ✅ Screen reader accessible
- ✅ Progressive enhancement (works without JS)

### Enhanced: data-page Navigation (With Parameters)

Page-name routing with rich parameter passing:

```html
<!-- Navigate by page name -->
<button data-page="settings">Settings Panel</button>
...
**Benefits:**
- ✅ Page name routing (semantic navigation)
- ✅ Rich parameter passing via JSON
- ✅ Dynamic routing independent of URL structure
...
class NavigationApp {
  async initialize() {
    ...
    // Make globally accessible for navigation
...
// Pages automatically handle both navigation types
class MyPage extends Page {
  on_params(params = {}, query = {}) {
    // Handle URL params: /users/123
    if (params.id) this.loadUser(params.id);
    
    // Handle data-page params: {"tab": "profile"}
    if (params.tab) this.showTab(params.tab);
    
    // Handle query params: ?sort=name
    if (query.sort) this.applySorting(query.sort);
  }
}
```
