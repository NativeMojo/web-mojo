...
- **DNS** — domains, DNS records, certificates, provider credentials, purchase ledger (`system/dns/*`, over django-mojo's `dnsman` app). The record editor is per-type structured (MX is priority+target, SRV four fields, CAA flags/tag/value), autofixes pasted input, checks CNAME collisions against the loaded zone, and shows a value diff before any write — both providers *replace* the whole record set, so an unshown removal is a silent data loss. Buying a domain is a search→quote→confirm→register wizard; the TLD comparison grid and similar-name suggestions need django-mojo ≥ v1.2.55 (older backends fall back to a single-name check with a note). **Registrant Contact** (`system/dns/registrant`) edits the ICANN contact registrations are filed under, in two scopes: the *house* contact used by every group without one of its own, and a specific *group's* contact. A group that merely inherits a contact is told so and never shown its values — that contact is the operator's personal name, address and phone. Needs django-mojo's portal-managed registrant API; an older backend says so and points at `DNSMAN_REGISTRANT_CONTACT` instead of erroring.
