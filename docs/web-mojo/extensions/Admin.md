...
The section fetches the resolved (inherited) config once from `GET /api/auth/config?group_uuid=<uuid>` to populate placeholders, and each field is seeded with the group's own override if present, else the inherited value. Edits **autosave** (debounced): the section diffs against that baseline and writes only the changed keys as a nested `{ metadata: { auth_config: {…} } }` via `model.save()` — django deep-merges the `metadata` JSONField, so sibling keys survive and untouched fields keep inheriting. An inline status line reports save progress; server validation errors also surface as a toast.
