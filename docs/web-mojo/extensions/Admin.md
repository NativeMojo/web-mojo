...
- **DNS** — domains, DNS records, certificates, provider credentials, purchase ledger (`system/dns/*`, over django-mojo's `dnsman` app). The record editor is per-type structured (MX is priority+target, SRV four fields, CAA flags/tag/value), autofixes pasted input, checks CNAME collisions against the loaded zone, and shows a value diff before any write — both providers *replace* the whole record set, so an unshown removal is a silent data loss. Buying a domain is a search→quote→confirm→register wizard; the TLD comparison grid and similar-name suggestions need django-mojo ≥ v1.2.55 (older backends fall back to a single-name check with a note).
...
- `view_dns` / `manage_dns` — the `system/dns/*` pages. `view_dns` renders read-only; every write control needs `manage_dns`. The `security` category covers both (mirroring dnsman's `VIEW_PERMS`/`SAVE_PERMS`). **Three surfaces are stricter than they look and the UI matches each deliberately:** *adopting a hosted zone* checks the literal `is_superuser` attribute, not the `admin` wildcard, because adoption hands a group control of a zone in the house account; *certificate key material* is never exposed by this UI at any permission level (that endpoint exists for serving hosts pulling with their own API key after a `certificate_updated` broadcast); and **WHOIS is gated on `manage_dns`, not `view_dns`** — the registrar returns the real registrant name, address, phone and email regardless of WHOIS privacy, so a read-only operator does not see the section at all.
...
3. **Registration** — an `enabled` toggle, `passkey_prompt` and `identity_field` selects, a `min_age` number, a `methods` multiselect, a fixed 6-row grid for `registration.fields` (an include / required / verify control per canonical field — `first_name`, `last_name`, `email`, `phone`, `dob`, `password`; `password` may be omitted for passwordless registration but is always required when included), and an **Extra fields** tag input for `registration.extra_fields`. The tag input collects non-canonical signup field names (e.g. `promo`, `ref`, `tracking`); it serializes to `[{name}]`, and django-mojo humanizes the label and defaults `required: false`. Names are sanitized client-side to the server's identifier rule (`[A-Za-z][A-Za-z0-9_]*`) and any name colliding with a canonical field is dropped, so the saved config always passes server validation. On the hosted register page each declared extra is captured silently from a matching URL query param or asked for as a text input, and captured values land on `user.metadata.registration`.

The section fetches the resolved (inherited) config once from `GET /api/auth/config?group_uuid=<uuid>` to populate placeholders, and each field is seeded with the group's own override if present, else the inherited value. A **Save** button diffs against that baseline and writes only the changed keys as a nested `{ metadata: { auth_config: {…} } }` via `model.save()` — django deep-merges the `metadata` JSONField, so sibling keys survive and untouched fields keep inheriting. Save is explicit (not autosave) because the server applies cross-field validation — `registration.fields` must include email or phone, and `login.methods` must be non-empty — so a mid-edit state is routinely invalid; those rules are checked client-side too, surfacing an inline message and a toast instead of a raw 400.
