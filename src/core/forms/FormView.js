...
class FormView extends View {
  ...
  initializePasswordFields() {
    ...
    // Enhance toggle buttons with press-and-hold reveal
    const toggleButtons = this.element.querySelectorAll('button[data-action="toggle-password"]');
    toggleButtons.forEach((btn) => {
      const getTarget = () => {
        const targetId = btn.getAttribute('data-target');
        if (targetId) {
          // Best effort query (IDs generated are simple, so direct use is fine)
          return this.element.querySelector('#' + targetId);
        }
        // Fallback: try to find an input sibling in the same input-group
        const group = btn.closest('.input-group');
        if (group) {
          return group.querySelector('input[type="password"], input[data-field-type="password"], input[type="text"]');
        }
        return null;
      };
  ...
  async onActionTogglePassword(event, element) {
    event.preventDefault();
    if (element?.dataset?.hold === 'true') {
      // Ignore click generated by press-and-hold gesture
      return;
    }
