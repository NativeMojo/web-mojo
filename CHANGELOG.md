...
- **Four documented events never existed.** `table:search`, `table:page`, `table:pagesize` and `filter:edit` appear nowhere as an `emit()` in `src/` — a listener on any of them has always been dead code. Removed from `TableView.md` and `TablePage.md`, with a note naming the real events (the toolbar lives on ListView, so it emits `list:search` / `list:page` / `list:pagesize`; `params-changed` is the umbrella signal). **Decision: docs follow code** — emitting the `table:*` trio as aliases was three lines, but nobody can be successfully using names that never fired, and aliasing would double the event surface permanently. Also removed the stale sources that kept regenerating the wrong names: `TablePage`'s commented-out `on('table:search')` examples, its dead `off('table:search')` / `off('table:page')` teardown calls, and `ListView`'s `filter:edit` JSDoc line (which contradicted a comment 3,000 lines below it saying *"do NOT emit `filter:edit` here"*).
- **`cell:save:error` is now re-emitted by TableView.** It was emitted only on the `TableRow`; `_createItemView` forwarded `cell:edit` / `cell:save` / `cell:cancel` and not the fourth, so the inline-editing error branch was unreachable from a table-level listener. Additive — no caller can regress.
- **Documented what the source actually does.** Corrected `cell:*` payload shapes (`{ row, model, column, oldValue, newValue }`, not `{ model, field, value }`); added the real undocumented events (`table:fullscreen:enter`/`:exit`, `row:expand:toggle`, `columns:change`, `columns:reset`, `filter:remove`, `filters:clear`, `preset:change`, `stat:change`, the seven `list:*`); added the undocumented column keys (`editable`, `editableOptions`, `autoSave`, `template`, `action`, `class`/`className`, `hideable`, and the `name`/`title`/`format` aliases); added `rowAction`, `showRefresh`, `showFullscreen`, `batchPanelTitle`, `persistSelection`, `title`/`eyebrow`, `toolbarRight` and the top-level `fontSize` fallback; and completed ListView's event tables (they listed 6 of ~24) plus `addFormDefaults`, `addRequiresActiveGroup`/`User` and the `formFields` fallback.
...
- **`mode: 'models'` refreshes only the rows already on screen.** One batched `id__in` request per tick (chunked at 200) carrying the collection's current params, merged into the **existing** model instances. No membership change, no re-sort, no pagination shift, no scroll jump — and no loading skeleton, because nothing emits `fetch:start`. Only the rows that changed re-render, through the ordinary model-`change` path.
- **`mode: 'collection'` is the default and the bare number is untouched.** `autoRefresh: 30` still means exactly what it meant: same 5s clamp, same `fetch()` with no param mutation, same indicator, same teardown, and zero extra markup or CSS. The object form's key is **`every`**, not `interval` — every `*Interval` in this codebase is milliseconds, so `interval: 30` would read as 30 ms.
- **New public `Collection.refreshModels(ids, { chunkSize, signal })`** → `{ changed: Map<id, string[]>, missing, ok }`. Deliberately not built on `fetch()`, which would abort an in-flight user request whose param key differs, flip bound views to their loading visual, and `add()` genuinely-new ids. It never calls `parse()` (that rewrites `meta`, corrupting "Showing N of M") and leaves `meta` / `params` / `lastFetchTime` / model order alone. Ids the server doesn't return are **left untouched**, not removed — they keep their last known values until a full refresh. Filters ride along on the request, so a row that filtered itself out is reported missing rather than silently re-included.
- **Changed rows flash** (`.mojo-row-flash`, ~1.2s tinted fade; `flash: false` opts out). Change detection diffs before setting, so an object field that re-serializes identically is not a change — a steady row never flashes. The tint animates `background-image` rather than `box-shadow`: Bootstrap paints table cells with an `inset 0 0 0 9999px` shadow, but `box-shadow` is already owned by the `rowStripe` severity bar on `td:first-child`, and keyframes outrank normal author declarations — a box-shadow flash would erase the severity bar for its whole duration. Dark companion + `prefers-reduced-motion` guard included; the CSS ships only when the flash is on.
- **The flash is models-mode only, by construction.** A collection refetch resets the collection, and reset rebuilds every item view — no row element survives for a highlight to land on.
- Every WM-034 pause condition (hidden/blurred, active selection, inline cell edit, open row menu) and the leak-proof teardown apply to both modes: the mode branch sits after the guards. Models mode adds an in-flight guard for overlapping ticks and discards a response that lands after the user changed the query context. `TablePage` forwards the object form through the existing `autoRefresh` key by reference — no TablePage change.
- Example: `components/table-view/live-watch`. Tests: new `Collection.refreshModels.test.js` (+9) and `ListView.autoRefreshModels.test.js` (+17), plus additive cases in `ListView.autoRefresh.test.js`, `ListView.interactions.test.js` and `TablePage.option-forwarding.test.js`.

### Admin · DNS management pages over django-mojo `dnsman` (#394)

Five new pages under `system/dns/*` — Domains, DNS Records, Certificates, Credentials, Purchases — plus a `DNS` sidebar group, gated on the new `view_dns` / `manage_dns` permissions (both now registered in `User.GRANULAR_PERMISSION_TABS` and mapped from the `security` category, mirroring dnsman's own `VIEW_PERMS`/`SAVE_PERMS`). Requires django-mojo with `mojo.apps.dnsman`; the TLD comparison grid, similar-name suggestions and the capability pre-gates need ≥ v1.2.55.

- **Record editor is structured per type, not a free-text box.** `RECORD_SPECS` drives one input per field — MX is (priority, target), SRV four, CAA (flags, tag, value) — which removes the wrong-field-count error class entirely. Existing records load through `parseRecordValue`, so editing an MX never means hand-editing `"10 mail.example.com"` as a string.
- **Autofix on paste, never silent.** Strips URL schemes and paths from hostname targets, square brackets from IPs, curly quotes, zero-width and non-breaking characters — and every change is listed above the fields. The load-bearing one removes user-added quotes from a TXT value: Route53 re-quotes and 255-chunks TXT itself, so a double-quoted value breaks SPF/DKIM and ACME validation **with no error surfaced anywhere**.
- **Validation mirrors the backend and fills what it leaves open.** Zone containment, apex NS/SOA and wildcard position match `services/dns.py`; on top of that, A holds IPv4 and AAAA holds IPv6 (with a one-click "change type" fix, because that value is right and only its type is wrong), CNAMEs cannot sit at the apex, hold more than one value, or collide with another record at the same name — checked against the already-loaded zone, so no extra request.
- **Diff before every destructive write.** Both providers replace the entire record set, so editing one value of a three-value set would otherwise destroy the other two silently. Any save that removes a value shows exactly what disappears first. The removal *warning* fires only on a net shrink — warning on an ordinary same-size swap would train people to click through the one that matters.
- **Buy a domain** — search → quote → confirm → register. A typed TLD never collapses the search grid: `example.com` and `example` return the same exact-match / other-TLDs / similar-names sections. Availability is tri-state throughout — a registry that did not answer never renders as "taken". The single-use confirm token lives in view state only and is cleared after the attempt and on unmount; a quote redeems exactly once, so the purchase is single-shot and never auto-retries.
- **Capability-driven, not hardcoded.** `GET /api/dnsman/config` supplies the provider matrix (so management-only is read from the server rather than testing `!== 'route53'`), the allowed record types, the price cap (an over-cap row is marked before selection instead of 400ing at quote), the certificate renewal window (which drives the expiry tones), and the ACME state — Request-certificate is disabled with a reason when ACME is unconfigured, and a persistent banner warns when issuance points at the Let's Encrypt **staging** directory, whose certificates are not publicly trusted.
- Certificate key material is not exposed by this UI at any permission level; adopting a hosted zone gates on the literal `is_superuser` attribute; WHOIS is gated on `manage_dns` rather than `view_dns` because the registrar returns real registrant PII regardless of privacy settings.
- Test runner gains `.not.toMatch`.
...
- **`filterPresets:` (WM-032)** — author-named filter bundles as a compact grouped segment (day-range SegmentControl family): inline beside day-range for ≤4 presets, scrollable row above the pills for 5+. One click applies the bundle (`setFilter` + `applyFilters`); mutual exclusion with toggle-off; active state *derived* from strict param matching (most-specific match wins when one preset's params are a subset of another's) so removing a pill or clearing filters deactivates the preset automatically; `'@me'` token resolves the active user at apply and match time. API: `applyPreset(key)` / `clearPreset()` / `getActivePreset()` + `preset:change` event.
- **`emptyState:` / `loadingStyle:` / `showResultCount:` (WM-033)** — rich empty states with a truly-empty vs no-filter-matches branch (filtered variant auto-offers Clear filters); shimmer skeleton rows echoing the real column/card silhouettes (size-capped, reduced-motion guard); "Showing N of M · filtered" summary in the pills row. `emptyMessage` remains the fallback. **Skeleton is the default loading visual** — pass `loadingStyle: 'spinner'` to restore the old spinner (the one deliberate exception to the no-option-no-change rule, scoped to the loading state).
- **`autoRefresh: <seconds>` (WM-034)** — interval refetch (5s floor) preserving current params; pauses while the tab is hidden/blurred, during row selection, inline cell edits, and open row menus; immediate refetch on focus resume; leak-proof across cached page revisits. A quiet toolbar indicator (muted `bi-arrow-repeat`, pulse on tick, tooltip with the cadence) shows it's on; `autoRefreshIndicator: false` hides it.
...
- `render()` calls made while a render is already in flight were silently dropped by the `isRendering` guard — with a fast (or synchronous) collection fetch, the `fetch:start`/`fetch:end` re-renders both landed mid-flight and the view stayed frozen on its initial loading visual forever (seen in the examples portal; the Show More flow had previously worked around the same seam). Requests now queue exactly one trailing render (latest args win) that runs after the in-flight pass completes; `await render()` awaits the trailing paint too. Regression-tested with a synchronously-emitting collection.
...
- **Every backend-supported delivery variant is offered, gated on what's on file.** Reset opens a delivery chooser — *Email link* / *Email 6-digit code* / *Text 6-digit code* (`method: 'link' | 'code'`, `channel: 'sms'`; there is no SMS-link variant — backend link mode is email-only). Magic link offers *Email* / *Text* (`method: 'sms'`). SMS options only appear when a phone number is on file (the backend silently no-ops SMS to a phone-less account), single-channel users get a plain confirm, and the identifier falls back from `email` to `phone_number` for email-less accounts. The Profile "Password" row gate widens from email-only to email-or-phone to match.
- **Revoke All Sessions no longer 404s** — `POST /api/user/<id>/sessions/revoke` (a route that never existed) is now the `POST /api/user/<id>` body `{ revoke_sessions: {} }` POST_SAVE_ACTION, the same pattern the view already used for `disable`/`reactivate`.
- **The dead Impersonate handler is deleted.** It targeted `/api/auth/impersonate` (no such route anywhere in django-mojo) and was wired to no UI. Real impersonation is tracked as a cross-repo pair (django-mojo one-time handoff-code endpoint + web-mojo UI).
- **Three admin actions the backend supported but the portal never surfaced:** **Resend Invite** (kebab; email on file + never logged in; `{ send_invite: true }`), **Reset MFA** (kebab; shown when `requires_mfa`; `{ disable_totp: true }` with an opt-in checkbox to also clear `requires_mfa` — default keeps it on, which is safe: login with `requires_mfa` and no enrolled methods proceeds without a challenge, so the user re-enrolls rather than locking out), and **Send verification email** (Profile email row, unverified emails; posts **`/api/auth/email/verify/send`** — the public admin-targetable route, *not* the similarly-named self-only `/api/auth/verify/email/send`).
- Tests: `test/unit/UserView.actions.test.js` (22 regression tests pinning the exact URL + payload per action variant, plus kebab gating), `test/unit/ContextMenu.test.js` (+6 gating tests). Test-infra: `UserView` registered in `simple-module-loader` with model-import stub mappings.

### Core · ContextMenu items support `permissions` (fail-closed) and `when(context)` gating (WM-027)

- **Behavior change — `permissions` keys on DetailView kebab items are now enforced.** `ContextMenu` (the component behind every DetailView header kebab and row menu) rendered all configured items unconditionally; the `permissions` key was only honored by Dialog's own header filter, which DetailViews opened with `header: false` never hit — so admin-gated items (Edit User, Change Password, Revoke All Sessions, …) were visible to every viewer. The new `ContextMenu.visibleItems()` enforces `permissions` via `View#checkPermissions` (any-of, **fail-closed** — same semantics as SideNavView sections) at render time, and a filtered item can no longer be dispatched through `onActionMenuItemClick` either. Menus that relied on the old leak now hide those items correctly.
- **New `when: (context) => bool` item predicate** — mirrors the `when:` API on DetailView header chips, evaluated against the menu's `context` (the model, in a DetailView kebab) on every render, so items track model state live (UserView's *Resend Invite* / *Reset MFA* use it). `ModalView.filterContextMenuItems` gained the same `when` support (evaluated against the body view's model) for API symmetry.
- Dividers left leading, trailing, or doubled after filtering are dropped; a fully-filtered menu renders nothing. Docs: `components/ContextMenu.md` rewritten for the new gating (the "ContextMenu has no permission system" caveat is gone).
...
- **Gating moved from construction time to render/navigation time.** Sections used to be filtered once in the constructor — before a parent/app was attached — which is exactly why the old code failed open ("app isn't available yet, allow"). All sections are now stored and visibility is decided when the nav renders or navigates: a gated section appears automatically once a permitted user resolves, `showSection()` refuses gated keys (programmatic calls included), `getSectionKeys()` returns only visible sections, a gated `activeSection` falls back to the first visible one, and divider labels whose whole group is gated are dropped.
- Regression tests in `test/unit/SideNavView.permissions.test.js` (fail-closed with no app / no user, any-of arrays, post-construction app attach, showSection refusal, active-section reconciliation, divider cleanup); verified live on GroupView (all 10 sections with a permitted user; Webhooks/Geofencing/Audit hidden with none). Docs: `components/SideNavView.md`. Flagged by the ITEM-023 post-build security review.

### Tooling · `npm run lint` now actually lints the tree (glob fix + real-baseline triage)

- **The lint script never recursed.** `"eslint src/**/*.js"` runs under npm's `/bin/sh`, which doesn't expand `**` — only 13 files (one level under `src/`) were ever linted; `src/extensions/` (the entire admin extension), `src/admin.js`, `src/core/models/`, and everything two-plus levels deep were silently skipped. The script is now `eslint src` (ESLint recurses natively; `lint:fix` likewise).
- **Real bugs surfaced by the first full run, fixed:** `EmailDomainTablePage` used `EmailDomainForms` without importing it — the Edit-AWS-Creds and Onboard actions on the email-domains admin page threw `ReferenceError` at runtime; a duplicate `precision` key in `forms/inputs/index.js`'s `createInput`; two dead self-assignments (plus a debug `console.log`) in `GroupSelectorButton.onBeforeRender`; three `no-promise-executor-return` violations in `TicketPanelView`; a useless rethrow-only try/catch in `FormView.saveModel`.
- **Deliberate config triage:** `bootstrap` declared a readonly global (Bootstrap's JS bundle); `/* global toastui | Prism | define */` headers on the docit/mojo-auth files that consume CDN/UMD globals; generated `src/extensions/map/countryCentroids.js` added to `ignorePatterns` (its duplicate ISO keys — BQ/ES/TF — are a generator artifact worth fixing at the source); the 20 intentional lazy `import()` sites in core (WebApp page/extension loaders, Modal's on-demand FormView/DataView, Model, PortalApp, TopNav) carry explicit `eslint-disable-next-line` annotations instead of weakening the no-dynamic-import rule; and four purely mechanical recommended rules (`no-useless-escape`, `no-case-declarations`, `no-prototype-builtins`, `no-empty`) are downgraded to warnings as tracked debt (~40 pre-existing sites in previously-unlinted code).
- **New baseline: 0 errors, 316 warnings** (was a fictitious "16/55" over 13 files). The warning pool is the debt register — promote the downgraded rules back to errors as files get cleaned.

### Core · Pipes on empty arrays no longer flip section rendering (`{{#x|bool}}` / `{{^x|bool}}`)

- **`dataFormatter.apply()`'s alternate-call-shape detection misfired on empty arrays.** The `apply(value, [...pipeNames])` shape is detected by "value is an array of strings" — and `[]` vacuously passes `every()`, so any pipe applied to an empty array reduced over zero formatter names and returned the seed: the formatter NAME string (truthy!). In templates this inverted both section forms whenever the value was an empty array — `{{#items|bool}}` rendered its body and `{{^items|bool}}` (the empty-state pattern Templates.md explicitly recommends) rendered nothing. The detection now requires a non-empty array, so `bool([])` runs the real formatter (false) and both section forms behave as documented. The alternate call shape itself is unchanged.
- Regression tests in `test/unit/Mustache-piped-sections.test.js`: direct `pipe`/`apply` on empty arrays, the alternate call shape still working, and section rendering across empty/non-empty arrays, booleans, and missing keys. Found while building the geofencing admin UI (ITEM-023) — its templates use plain `{{^x}}` and are unaffected either way.

### Admin · Geofencing section — rules editor, simulator, blocks log, exemptions (ITEM-023)

- **New admin page `system/security/geofencing`** (Network Security › Geofencing) over the django-mojo ≥ v1.2.42 geofence config plane. Four tabs under an at-a-glance posture header (enforcement status, rules source, per-scope fail posture, cache TTL, exemption counts, last change, protected-endpoints list):
  - **Rules** — plain-language platform rules editor (country allow/block by name, blocked US states multiselect, anonymizer toggles) writing ONLY through validated `POST /api/geo/rules`; server 400s surface inline verbatim. Rules using DSL shapes the guided editor can't represent (eq, region allow-lists, require-flags) flip to a validated JSON view instead of being silently rewritten; the guided↔JSON toggle carries unsaved edits both ways. A "Remove portal override" action (confirm + `DELETE`) appears only for DB-sourced rules, and a deploy-file banner explains override semantics when `source` is `conf`. Change history renders from `geofence_config` incident events (quietly gated on security-events access).
  - **Simulator** — location (country/state/abuse flags) or IP what-if via `POST /api/geo/simulate`; decision card gives the plain-language verdict, the exempt case's `would_block` shadow outcome, and an engine-off staged-preview notice. Endpoint-scope options are **derived from the deployment's `enforced_endpoints`** — scopes are deployment-defined strings, never hardcoded (django-mojo core ships only `auth`).
  - **Blocks log** — `geofence_block` incident events with plain-language reasons, severity striping (5 abuse/fail-closed · 6 fail-open pass · 7 rule error), country/region/IP/level filters, and an activity strip (MetricsMiniChartWidget tiles + MetricsChart over the backend-recorded `geofence:blocks` / `geofence:exempt` slugs) with PNG export for compliance evidence.
  - **Exemptions** — CIDR allowlist editor (full-replace `POST /api/geo/allowlist`, expired entries shown inactive, first-bad-entry 400s surfaced), per-IP whitelist with reason/expiry via the standard geoip `whitelist`/`unwhitelist` actions, and the read-only `bypass_geofence` holders list with permission-vs-superuser source badges.
- **GroupView gains a "Geofencing" sidebar section** editing `Group.metadata.geofence` over a read-only platform floor, with the effective policy shown clause-by-clause tagged Platform / This group. Saves build a **merge-safe null-diff payload** (django deep-merges metadata JSONFields and `null` deletes a key; nested `__replace` is unsupported) so removing a constraint or flipping an operator actually clears the old value — `buildGroupRulePayload` is unit-tested for exactly this.
- **Permissions are global-only by design:** page, menu item, and section gate on `sys.view_geofence` / `sys.manage_geofence` / `sys.security` (the `sys.` prefix skips group/member grants, matching the backend's global-grant enforcement); `view_geofence` alone gets a true read-only render.
- Pure rule↔form mapping, plain-language reason texts for all 15 decision codes (with safe fallback), simulate-body builder, and the group payload builder live in dependency-free `geofenceData.js` with 34 unit tests (`test/unit/GeofenceData.test.js`). Docs: `extensions/Admin.md`. Filed as the UI for django-mojo#ITEM-017 (MojoVerify payment-compliance evidence + WMX state deny list).
