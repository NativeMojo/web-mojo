...
- **Default visual treatment** is a typographic mark + fading rule composition, not a generic eyebrow: a sharp 4×4 brand-tinted square anchors a bold uppercase emphasis-color label, followed by a directional gradient hairline that's solid for ~24px after the label and fades to transparent. Spacing creates the section break — 1.5rem breathing room above each group, tight 0.5rem to the first item below. CSS-only, all Bootstrap tokens — light + dark from a single block. Per-instance accent override via `--list-group-header-accent` (e.g. `style="--list-group-header-accent: var(--bs-success)"` for an "ACTIVE" group). TableView shape mirrors the composition inside `<th colspan="N">` using cell padding for vertical rhythm.
