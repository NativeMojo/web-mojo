...
- **Added:** `variant` constructor option on `TabView` (`src/core/views/navigation/TabView.js`). Eight values — `'minimal'` (new default), `'traditional'`, `'underline'`, `'underline-all'`, `'pills'`, `'pills-solid'`, `'segmented'`, `'btn-group'` (with aliases `'buttongroup'` and `'btngroup'`). Each maps to a `tab-view-variant-*` hook class on the rendered `<ul>` (and the dropdown wrapper when the bar collapses); all visual differences live in `core.css`. Aliases resolve to the canonical name. Unknown values warn and fall back to `'minimal'`.
- **Behavior change:** the default tab style changed from `nav nav-tabs mb-3` (Bootstrap nav-tabs underline) to `'minimal'` (text-only, primary on active, no border). The previous look is one line away — pass `variant: 'traditional'` to restore it. `'traditional'` resolves to the literal `'nav nav-tabs mb-3'` class string for bit-for-bit back-compat with pre-variant code.
- **Variant family overview:**
  - **Text-style** (separated buttons, no group container): `minimal`, `traditional`, `underline`, `underline-all`, `pills`, `pills-solid`.
  - **Group-style** (connected, all tabs share the container's chrome): `segmented` (neutral grey container, primary active), `btn-group` (Bootstrap btn-group with hairline borders, primary active). `btn-group` is the visual sibling of `SegmentControl` — same look, different job (view-swap vs value-pick).
...
- **CSS:** variants section in `src/core/css/core.css` (after the existing TabView block). Uses Bootstrap surface tokens (`--bs-body-color`, `--bs-secondary-color`, `--bs-primary`, `--bs-secondary-bg`, `--bs-tertiary-bg`, `--bs-primary-bg-subtle`, `--bs-border-color`, etc.) with all `[data-bs-theme="dark"]` refinements clustered at the bottom of the section per `.claude/rules/theming.md`.
- **Public-API export:** added `SegmentControl` to the `web-mojo` named exports in `src/index.js` (it was already documented as `import { SegmentControl } from 'web-mojo'` but the export was missing).
- **Tests:** `test/unit/TabView.test.js` covers default variant, each of the eight named variants' resolved `tabsClass`, alias resolution (`buttongroup` / `btngroup` → `btn-group`), unknown-variant warn-and-fallback, and `tabsClass` precedence over `variant`.
- **Docs:** `docs/web-mojo/components/TabView.md` Variants section lists all eight variants with light/dark behavior, notes the default change with the `traditional` migration line, and flags that `'underline'` is *not* the legacy nav-tabs look (use `'traditional'` for that).
- **Example portal:** `components/tab-view/variants` route (`TabViewVariantsExample.js`) — pick a variant via `SegmentControl`, the same TabView re-renders with the new look. The picker itself uses the `btn-group` look so the `btn-group` variant of TabView visually matches its surrounding picker.
