...
- **`src/extensions/admin/shortlinks/ShortLinkView.js`** rewritten end-to-end against the Wave 1 framework primitives and Wave 3 design language:
  - **Mustache + DataFormatter throughout.** Every section view (`ShortLinkOverviewSection`, `ShortLinkConfigurationSection`, `ShortLinkOgSection`, `ShortLinkMetadataSection`) now uses a Mustache string template with `{{model.field}}`, `{{model.expires_at|datetime}}`, `{{#flag|bool}}…{{/}}` blocks, and a small set of `getter` properties for derived values (`hasOg`, `hasUrl`, `sourceLabel`, `_flat`, etc.). The hand-rolled `_buildTemplate()` string-concatenation methods and the local `escapeHtml()` helper are gone — Mustache's auto-escape and view-level getters carry every per-template field.
  - **Overview KPIs** are four `MetricCard` instances at the default size (no `metric-card-lg`): Hits · 30d, Hits · 7d, Today, Top country. The `summarizeClicks(...)` helper rolls up the shared 30-day clicks collection into the values, with a single in-memory pass that also tracks top country (reads `country` / `geo.country` / `ip_info.country` defensively).
  - **Slack/iMessage preview** is now a borderless tinted region (`.sl-preview` family in `admin.css`) inside Overview — NOT wrapped in a `.card` per Wave 3 design language. Renders with og:image / og:title / og:description when set, falls back to a gradient placeholder + Bootstrap-Icons link glyph when unset, and shows a small info alert explaining the auto-scrape behaviour when no OG metadata exists.
  - **Hand-rolled SVG sparkline + 30-day stats card removed.** That dual-card row was a `.card` stack with a hand-built `<svg>` — replaced by the four `MetricCard`s above and the dedicated Metrics section's `MetricsChart`.
  - **Configuration section** drops three `.detail-field-card` wrappers in favor of three `.detail-section-eyebrow` subsections (Destination / Tracking / Lifecycle) of `.detail-flat-row` rows. Each eyebrow exposes a `.detail-section-action` pencil to open the edit form. No more inline `<button class="btn btn-sm btn-link">` headers.
  - **OG / Social section** is now a labeled flat-row layout (one row per og:* / twitter:* key) instead of a card-with-header + nested `DataView`. The empty-state is a single muted paragraph; the eyebrow pencil routes to the same focused OG edit dialog.
  - **Metadata section** wraps `@core/views/data/KnownFieldsCard.js` — the per-section hand-rolled `<pre>` JSON dump is gone; raw metadata JSON now lives in the framework's collapsible `<details>` block. The eyebrow exposes a pencil to open the JSON edit modal. Empty state is the framework's stable "—" placeholder.
  - **Metrics section** kept (already used `MetricsChart`); unchanged structurally other than the eyebrow swap. The disabled-tracking empty state ("Click tracking is disabled — enable…") is now a borderless `alert alert-info` block instead of a card body.
  - **Click History TableView** refined: `track_clicks=false` empty message now explains *why* the table is empty (and how to enable tracking) instead of the original "Click tracking is disabled for this shortlink." one-liner.
  - **Header `auxFn`** replaces the synthetic-`_subtitle` round-trip with the standard `(model) => htmlString` slot. Renders a state-aware right-gutter readout (presence dot + main label + muted relative time): "42 hits · updated 3m ago" for active links, "Disabled" / "Expired" tone-coded otherwise. Trusted HTML — model fields escaped via `MOJOUtils.escapeHtml(...)` before composition. Header `subtitleFn` retains the "→ {url}" line.
...
- **`src/extensions/admin/css/admin.css`** — appended a new `ShortLinkView · Slack / iMessage preview tile` block with `.sl-preview`, `.sl-preview-thumb`, `.sl-preview-body`, `.sl-preview-domain`, `.sl-preview-title`, `.sl-preview-desc`. Surfaces use Bootstrap tokens (`--bs-secondary-bg`, `--bs-body-color`, `--bs-secondary-color`, `--bs-primary`, `--bs-info`) so dark theme works automatically; no per-component dark override needed. The thumb gradient + `color: #fff` are content-on-tint literals (not surfaces) so they stay readable in both themes.
- **Tests:** `test/unit/ShortLinkView.test.js` adds seven regression cases — Overview mounts four MetricCard instances at default size, Overview's Slack/iMessage preview renders with the `.sl-preview` class and NO `.card` wrapper, Metadata section mounts a KnownFieldsCard once navigated to, the view registers the 8-entry `sectionsConfig` (Overview / Configuration / [Activity] / ClickHistory / Metrics / [Detail] / OgSocial / Metadata), the dialog does NOT force `size: 'xl'`, the header `auxFn` renders the right-gutter readout slot, and the Click History empty message is `track_clicks`-aware.
- **Test loader** (`test/utils/simple-module-loader.js`) registers `ShortLinkView` and the `@core/models/ShortLink` + `@ext/charts/index` import paths so tests can stub `global.ShortLinkModelsStub` and `global.ChartsStub`.
- No public API changes. All 808 unit tests pass; lint clean on touched files.
